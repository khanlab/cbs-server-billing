...
class PowerUsersRecord:
    ...
    def describe_user(self, last_name, period_start, period_end):
        ...
        orig_row = self.power_user_df.loc[
            ...
            :]
        orig_dates = [orig_row.loc[:, "start_timestamp"].iloc[0],
        ...
        if len(relevant_updates) == 0:
            if (
                    orig_power_user
                    and (pd.isna(orig_dates[1])
                         or (orig_dates[1] >= period_start))
        ...
        if len(end_date_updates) > 0:
            newest_end_date = end_date_updates.loc[
                end_date_updates["timestamp"].idxmax(),
                "new_end_timestamp"]
        if len(status_updates) > 0:
            currently_power_user = status_updates.loc[
                status_updates["timestamp"].idxmax(),
                "new_power_user"]
        ...
        # Status has changed
        end_date = (newest_end_date if len(end_date_updates) > 0
                    else orig_dates[1])
        ...
        if ((start_date < period_end)
                and (pd.isna(end_date) or (end_date > period_start))):
...
def load_user_update_df(user_update_form_path):
    ...
    user_update_df = user_update_df.rename(
        columns={
            ...
            "Request access to additional datashare (specify PI's last name)":
                "additional_datashare",
            "Update contract end date (account expiration)":
                "new_end_timestamp",
            "Change account type":
                "new_power_user",
            "List projects for which you need security access":
                "new_projects",
...
def generate_all_pi_bills(paths,
                          quarter_start_iso,
                          out_dir):
...
if __name__ == "__main__":
    ...
    parser.add_argument("user_update_form",
                        type=str,
